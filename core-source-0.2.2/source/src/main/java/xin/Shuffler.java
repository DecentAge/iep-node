...
public final class Shuffler {
    ...
    private static final int MAX_SHUFFLERS = Xin.getIntProperty("xin.maxNumberOfShufflers");
    ...
    private static void scheduleExpiration(Shuffling shuffling) {
        int expirationHeight = Xin.getBlockchain().getHeight() + 720;
    ...
    private volatile XinException.NotCurrentlyValidException failureCause;
    ...
    public XinException.NotCurrentlyValidException getFailureCause() {
        return failureCause;
    }
    ...
    private void submitTransaction(Attachment.ShufflingAttachment attachment) {
        ...
        try {
            Transaction.Builder builder = Xin.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, 0,
                    (short) 1440, attachment);
            builder.timestamp(Xin.getBlockchain().getLastBlockTimestamp());
            Transaction transaction = builder.build(secretPhrase);
            failedTransaction = null;
            failureCause = null;
            Account participantAccount = Account.getAccount(this.accountId);
            if (participantAccount == null || transaction.getFeeTQT() > participantAccount.getUnconfirmedBalanceTQT()) {
                failedTransaction = transaction;
                failureCause = new XinException.NotCurrentlyValidException("Insufficient balance");
