...
final class BlockchainProcessorImpl implements BlockchainProcessor {
    ...
    private final Runnable getMoreBlocksThread = new Runnable() {
        ...
        private void downloadPeer() throws InterruptedException {
            ...
            try {
                ...
                int numberOfForkConfirmations = blockchain.getHeight() > Constants.LAST_CHECKSUM_BLOCK_HEIGHT - 720 ?
                ...
                try {
                    if (betterCumulativeDifficulty.compareTo(blockchain.getLastBlock().getCumulativeDifficulty()) <=
                            0) {
                        return;
                    }
                    ...
                    for (Peer otherPeer : connectedPublicPeers) {
                        ...
                        if (otherPeerResponse == null ||
                                (otherPeerCumulativeDifficulty = (String) response.get("cumulativeDifficulty")) ==
                                        null) {
                            continue;
                        }
                    ...
                    if (blockchain.getLastBlock().getId() != lastBlockId) {
                        ...
                        Logger.logMessage("Downloaded " + numBlocks + " blocks in "
                                ...
                                + totalTime * (lastBlockchainFeederHeight - blockchain.getHeight()) /
                                ((long) totalBlocks * 1000 * 60) + " min left");
                    } else {
        ...
        private void downloadBlockchain(final Peer feederPeer, final Block commonBlock, final int startHeight)
            ...
            if (slowestPeer != null && connectedPublicPeers.size() >= Peers.maxNumberOfConnectedPublicPeers &&
            ...
            try {
                ...
                for (int index = 1;
                     index < chainBlockIds.size() && blockchain.getHeight() - startHeight < 720; index++) {
    ...
    private static class GetNextBlocks implements Callable<List<BlockImpl>> {
        ...
        public List<BlockImpl> call() {
            ...
            if (nextBlocks.size() > 36) {
                Logger.logDebugMessage(
                        "Obsolete or rogue peer " + peer.getHost() + " sends too many nextBlocks, blacklisting");
    ...
    private final Listener<Block> checksumListener = block -> {
        ...
        if (Constants.BLOCK_CHECKSUMS.containsKey(block.getHeight())) {
            Logger.logErrorMessage("Calculating checksum for block " + block.getHeight());
            BlockChecksum checksum = Constants.BLOCK_CHECKSUMS.get(block.getHeight());
            if (!verifyChecksum(checksum.getChecksum(), checksum.getFrom(), checksum.getTo())) {
                popOffTo(checksum.getFrom());
            }
        }
    ...
    public int getMinRollbackHeight() {
        return trimDerivedTables ?
                (lastTrimHeight > 0 ? lastTrimHeight : Math.max(blockchain.getHeight() - Constants.MAX_ROLLBACK, 0)) :
                0;
    }
        ...
        try (Connection con = Db.db.getConnection()) {
            ...
            transactionList.forEach(prunableTransaction -> {
                ...
                if ((prunableTransaction.hasPrunableAttachment() &&
                        ...
                        PrunableMessage.isPruned(id, prunableTransaction.hasPrunablePlainMessage(),
    ...
    private boolean addGenesisBlock() {
        ...
        try {
            ...
            BlockImpl genesisBlock =
                    new BlockImpl(-1, 0, 0, Constants.MAX_BALANCE_TQT, 0, transactions.size() * 128, digest.digest(),
                            Genesis.CREATOR_PUBLIC_KEY, new byte[64], Genesis.GENESIS_BLOCK_SIGNATURE, null,
                            transactions, byteATs);
        ...
        try {
            ...
            try {
                ...
                if (nextHitTime > 0 && block.getTimestamp() > nextHitTime + 1) {
                    String msg =
                            "Rejecting block " + block.getStringId() + " at height " + previousLastBlock.getHeight()
                ...
                validatePhasedTransactions(previousLastBlock.getHeight(), validPhasedTransactions,
                        invalidPhasedTransactions, duplicates);

                validateTransactions(block, previousLastBlock, curTime, duplicates,
                        previousLastBlock.getHeight() >= Constants.LAST_CHECKSUM_BLOCK_HEIGHT);
    ...
    private void validatePhasedTransactions(int height, List<TransactionImpl> validPhasedTransactions,
                ...
                for (TransactionImpl phasedTransaction : phasedTransactions) {
                    ...
                    try {
                        ...
                        if (!phasedTransaction.attachmentIsDuplicate(duplicates, false)) {
                            ...
                            Logger.logDebugMessage(
                                    "At height " + height + " phased transaction " + phasedTransaction.getStringId() +
    ...
    private void validate(BlockImpl block, BlockImpl previousLastBlock, int curTime) throws BlockNotAcceptedException {
        ...
        if (block.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT) {
            Logger.logWarningMessage(
                    "Received block " + block.getStringId() + " from the future, timestamp " + block.getTimestamp()
                            + " generator " + Long.toUnsignedString(block.getGeneratorId()) + " current time " +
                            curTime + ", system clock may be off");
        ...
        if (block.getVersion() != 1 &&
                !Arrays.equals(Crypto.sha256().digest(previousLastBlock.bytes()), block.getPreviousBlockHash())) {
        ...
        // /* rollback stub */
        if (Constants.ROLLBACK_TAG) {
            if (Constants.ROLLBACK_BLOCK.equals(block.getId()) &&
                    previousLastBlock.getHeight() == (Constants.ROLLBACK_HEIGHT - 1)) {
                throw new BlockNotAcceptedException("Rollback :", block);
            }
        }

        if (!block.verifyGenerationSignature()) {
            Account generatorAccount = Account.getAccount(block.getGeneratorId());
            long    generatorBalance = generatorAccount == null ? 0 : generatorAccount.getEffectiveBalanceTKN();
            throw new BlockNotAcceptedException(
    ...
    private void validateTransactions(BlockImpl block, BlockImpl previousLastBlock, int curTime,
                                      Map<TransactionType, Map<String, Integer>> duplicates,
        ...
        for (TransactionImpl transaction : block.getTransactions()) {
            ...
            if (fullValidation) {
                ...
                if (transaction.getTimestamp() > block.getTimestamp() + Constants.MAX_TIMEDRIFT) {
                    throw new TransactionNotAcceptedException(
                            "Invalid transaction timestamp " + transaction.getTimestamp()
                ...
                if (transaction.referencedTransactionFullHash() != null) {
                    if ((previousLastBlock.getHeight() < Constants.REFERENCED_TRANSACTION_FULL_HASH_BLOCK
                            && !TransactionDb
                            .hasTransaction(Convert.fullHashToId(transaction.referencedTransactionFullHash()),
        ...
        AT_Block atBlock = AT_Controller.getCurrentBlockATs(Constants.MAX_PAYLOAD_LENGTH - (int) payloadLength,
                previousLastBlock.getHeight() + 1);

        byte[] byteATs = atBlock.getBytesForBlock();

        //Subtract at fees from block fee so that it corresponds to transactions fee
        //digesting AT Bytes
        if (AT.isEnabled() && byteATs != null) {
        ...
        if (Subscription.isEnabled()) {
            ...
            calculatedTotalFee += Subscription.calculateFees(block.getTimestamp());

        }

        // Add bad block here to handle fee/amount advanced tx errors

        if (calculatedTotalAmount != block.getTotalAmountTQT()) {
            throw new BlockNotAcceptedException("Total amount don't match transaction totals", block);
        }

        if (calculatedTotalFee != block.getTotalFeeTQT()) {
            throw new BlockNotAcceptedException("Total fee don't match transaction totals", block);
        }
        ...
        int transactionPayloadLength =
                block.getPayloadLength() - (block.getBlockATs() == null ? 0 : block.getBlockATs().length);

        if (hasPrunedTransactions ? payloadLength > block.getPayloadLength() :
                payloadLength != transactionPayloadLength) {
            throw new BlockNotAcceptedException(
                    "Transaction payload length " + payloadLength + " does not match block payload length "
                            + transactionPayloadLength, block);
        }

    }

    private void accept(BlockImpl block, List<TransactionImpl> validPhasedTransactions,
                        List<TransactionImpl> invalidPhasedTransactions,
                        ...
                        if (result != null && result.isApproved()) {
                            ...
                            phasingVoteCasting.getTransactionFullHashes().forEach(hash -> {
                                ...
                                if (phasingPoll.allowEarlyFinish() &&
                                    ...
                                    possiblyApprovedTransactions
                                            .add(TransactionDb.findTransaction(phasingPoll.getId()));
                                }
                            });
                        }
                    }
                ...
                possiblyApprovedTransactions.forEach(transaction -> {
                    if (PhasingPoll.getResult(transaction.getId()) == null) {
                        try {
                            ...
                            Logger.logDebugMessage("At height " + block.getHeight() + " phased transaction " +
    ...
    SortedSet<UnconfirmedTransaction> selectUnconfirmedTransactions(
        ...
        try (FilteringIterator<UnconfirmedTransaction> unconfirmedTransactions = new FilteringIterator<>(
                ...
                transaction -> hasAllReferencedTransactions(transaction.getTransaction(), transaction.getTimestamp(),
                        0))) {
        ...
        while (payloadLength <= Constants.MAX_PAYLOAD_LENGTH &&
            ...
            for (UnconfirmedTransaction unconfirmedTransaction : orderedUnconfirmedTransactions) {
                ...
                if (!EconomicClustering.verifyFork(transaction)) {
                    Logger.logDebugMessage("Including transaction that was generated on a fork: " + transaction
                    .getStringId()
                            + " ecBlockHeight " + transaction.getECBlockHeight() + " ecBlockId " + Convert
                            .toUnsignedLong(transaction.getECBlockId()));
                    //continue;
                }
                */
    ...
    void generateBlock(String secretPhrase, int blockTimestamp) throws BlockNotAcceptedException {
        ...
        if (blockchain.getHeight() >= Constants.PHASING_BLOCK) {
            try (DbIterator<TransactionImpl> phasedTransactions = PhasingPoll
                    .getFinishingTransactions(blockchain.getHeight() + 1)) {
                for (TransactionImpl phasedTransaction : phasedTransactions) {
                    try {
                        phasedTransaction.validate();
        ...
        AT.clearPendingFees();
        AT.clearPendingTransactions();
        AT_Block atBlock = AT_Controller
                .getCurrentBlockATs(Constants.MAX_PAYLOAD_LENGTH - payloadLength, previousBlock.getHeight() + 1);
        byte[] byteATs = atBlock.getBytesForBlock();

        //digesting AT Bytes
        if (byteATs != null) {
            payloadLength += byteATs.length;
            totalFeeTQT += atBlock.getTotalFees();
            totalAmountTQT += atBlock.getTotalAmount();
        }
        ...
        BlockImpl block =
                new BlockImpl(getBlockVersion(previousBlock.getHeight()), blockTimestamp, previousBlock.getId(),
                        totalAmountTQT, totalFeeTQT, payloadLength,
        ...
        try {
            ...
            Logger.logDebugMessage("Account " + Long.toUnsignedString(block.getGeneratorId()) + " generated block " +
                    block.getStringId()
                    + " at height " + block.getHeight() + " timestamp " + block.getTimestamp() + " fee " +
                    ((float) block.getTotalFeeTQT()) / Constants.ONE_XIN);
    ...
    private void scan(int height, boolean validate, boolean shutdown) {
        ...
        try {
            ...
            if (height > 0 && height < getMinRollbackHeight()) {
                Logger.logMessage("Rollback to height less than " + getMinRollbackHeight() +
            ...
            try (Connection con = Db.db.getConnection();
                 PreparedStatement pstmtSelect = con.prepareStatement(
                         "SELECT * FROM block " + (height > 0 ? "WHERE height >= ? " : "") + "ORDER BY db_id ASC");
                ...
                if (height > blockchain.getHeight() + 1) {
                    Logger.logMessage("Rollback height " + (height - 1) + " exceeds current blockchain height of " +
                            blockchain.getHeight() + ", no scan needed");
                ...
                try (ResultSet rs = pstmtSelect.executeQuery()) {
                    while (rs.next()) {
                        try {
                            ...
                            List<TransactionImpl>                      validPhasedTransactions   = new ArrayList<>();
                            List<TransactionImpl>                      invalidPhasedTransactions = new ArrayList<>();
                            validatePhasedTransactions(blockchain.getHeight(), validPhasedTransactions,
                                    invalidPhasedTransactions, duplicates);
                            if (validate && currentBlockId != Genesis.GENESIS_BLOCK_ID) {
                                ...
                                for (TransactionImpl transaction : currentBlock.getTransactions()) {
                                    ...
                                    if (currentBlock.getHeight() > Constants.TQT_BLOCK
                                            ...
                                            TransactionImpl.newTransactionBuilder(transactionBytes).build().bytes())) {
                                        throw new XinException.NotValidException(
                            ...
                            Logger.logDebugMessage(
                                    "Applying block " + Long.toUnsignedString(currentBlockId) + " at height "
