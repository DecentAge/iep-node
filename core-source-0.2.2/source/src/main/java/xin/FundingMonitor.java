 ...
 * <p>
 * XIN, ASSET and CURRENCY balances can be monitored.  If a balance falls below the threshold, a transaction
...
public final class FundingMonitor {
     ...
     */
    private static final int MAX_MONITORS = Xin.getIntProperty("xin.maxNumberOfMonitors");
     ...
     * @param holdingId    Asset or Currency identifier, ignored for token monitor
    ...
    private FundingMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        this.holdingId = (holdingType != HoldingType.XIN ? holdingId : 0);
    ...
    public static boolean startMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        // instead of during initialization so we don't start the monitor thread if it
        // won't be used.
        //
        init();
        ...
        synchronized (monitors) {
            ...
            while (monitorIt.hasNext()) {
                ...
                if (monitor.holdingType == holdingType && monitor.property.equals(property) &&
                        (holdingType == HoldingType.XIN || monitor.holdingId == holdingId) &&
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            Xin.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (Xin.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
                                ...
                                switch (monitoredAccount.monitor.holdingType) {
                                    case XIN:
                                        processXinEvent(monitoredAccount, targetAccount, fundingAccount);
                                        break;
                                    case ASSET:
     ...
     * @throws XinException Unable to create transaction
     */
    private static void processXinEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
            throws XinException {
        FundingMonitor monitor = monitoredAccount.monitor;
        if (targetAccount.getBalanceTQT() < monitoredAccount.threshold) {
            Transaction.Builder builder = Xin.newTransactionBuilder(monitor.publicKey,
                    monitoredAccount.amount, 0, (short) 1440, Attachment.ORDINARY_PAYMENT);
            builder.recipientId(monitoredAccount.accountId)
                    .timestamp(Xin.getBlockchain().getLastBlockTimestamp());
            Transaction transaction = builder.build(monitor.secretPhrase);
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeTQT()) > fundingAccount.getUnconfirmedBalanceTQT()) {
                ...
                monitoredAccount.height = Xin.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("Token funding transaction %s for %f token submitted from %s to %s",
                        transaction.getStringId(), (double) monitoredAccount.amount / Constants.ONE_XIN,
    ...
    private static void processAssetEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
        ...
        if (fundingAsset == null || fundingAsset.getUnconfirmedQuantityQNT() < monitoredAccount.amount) {
            ...
            builder.recipientId(monitoredAccount.accountId)
                    .timestamp(Xin.getBlockchain().getLastBlockTimestamp());
            Transaction transaction = builder.build(monitor.secretPhrase);
            if (transaction.getFeeTQT() > fundingAccount.getUnconfirmedBalanceTQT()) {
                Logger.logWarningMessage(String.format("Funding account %s has insufficient funds; funding transaction discarded",
                        monitor.accountName));
            } else {
                Xin.getTransactionProcessor().broadcast(transaction);
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            long balance = account.getBalanceTQT();
            //
            // Check the token balance for monitored accounts
            //
            synchronized (monitors) {
                List<MonitoredAccount> accountList = accounts.get(account.getId());
                if (accountList != null) {
                    accountList.forEach((maccount) -> {
                        if (maccount.monitor.holdingType == HoldingType.XIN && balance < maccount.threshold &&
