...
public final class AT extends AT_Machine_State {
    ...
    public static Collection<Long> getAllATIds() {
      ...
      // add getAllATs - limits and order
    public static Collection<Long> getAllATIdsSorted() {

        try (

        Connection con = Db.db.getConnection();
        PreparedStatement pstmt = con.prepareStatement("SELECT id FROM at WHERE latest = TRUE order by height DESC"
    ...
    public static boolean isATNameTaken(String name) {
        int count = atTable.getCount(new DbClause.StringClause("lower(name)", name.toLowerCase()));
        return count != 0;
    }
