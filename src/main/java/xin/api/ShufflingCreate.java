...
public final class ShufflingCreate extends CreateTransaction {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) throws XinException {
        ...
        if (Xin.getBlockchain().getHeight() < Constants.SHUFFLING_ACTIVATION_BLOCK) {
            throw new XinException.NotYetEnabledException(
                    "shuffling will be enabled at block " + Constants.SHUFFLING_ACTIVATION_BLOCK);
        }
        ...
        if (holdingType == HoldingType.XIN && amount < Constants.SHUFFLING_DEPOSIT_TQT) {
            return JSONResponses.incorrect("amount",
                    "Minimum shuffling amount is " + Constants.SHUFFLING_DEPOSIT_TQT / Constants.ONE_XIN + " XIN");
        }
        byte participantCount =
                ParameterParser.getByte(req, "participantCount", Constants.MIN_NUMBER_OF_SHUFFLING_PARTICIPANTS,
                        Constants.MAX_NUMBER_OF_SHUFFLING_PARTICIPANTS, true);
        short      registrationPeriod = (short) ParameterParser
                .getInt(req, "registrationPeriod", 0, Constants.MAX_SHUFFLING_REGISTRATION_PERIOD, true);
        Attachment attachment         =
                new Attachment.ShufflingCreation(holdingId, holdingType, amount, participantCount, registrationPeriod);
