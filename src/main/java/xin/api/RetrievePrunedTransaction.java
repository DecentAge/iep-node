...
import static xin.api.JSONResponses.UNKNOWN_TRANSACTION;
...
public class RetrievePrunedTransaction extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) throws ParameterException {
        ...
        Transaction transaction = Xin.getBlockchain().getTransaction(transactionId);
        if (transaction == null) {
            return UNKNOWN_TRANSACTION;
        }
        transaction = Xin.getBlockchainProcessor().restorePrunedTransaction(transactionId);
        if (transaction == null) {
