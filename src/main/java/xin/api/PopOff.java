...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Xin.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Xin.getBlockchainProcessor().popOffTo(Xin.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Xin.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Xin.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Xin.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
