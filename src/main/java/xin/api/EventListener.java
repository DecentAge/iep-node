...
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {
     ...
     * Maximum event users
     */
    static final int maxEventUsers = Xin.getIntProperty("xin.apiMaxEventUsers");

    /**
     * Event registration timeout (seconds)
     */
    static final int eventTimeout = Math.max(Xin.getIntProperty("xin.apiEventTimeout"), 15);

    /**
     * Blockchain processor
     */
    static final BlockchainProcessor blockchainProcessor = Xin.getBlockchainProcessor();

    /**
     * Transaction processor
     */
    static final TransactionProcessor transactionProcessor = Xin.getTransactionProcessor();
     ...
     * Xin event listeners
     */
    private final List<XinEventListener> xinEventListeners = new ArrayList<>();
     ...
     * <p>
     * Xin event listeners will be added for the specified events
     *
     * @param eventRegistrations List of Xin event registrations
     ...
     *
     * @param eventRegistrations Xin event registrations
     * @throws EventListenerException Invalid Xin event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<XinEventListener> it = xinEventListeners.iterator();
                while (it.hasNext()) {
                    XinEventListener listener = it.next();
                ...
                if (addListener) {
                    XinEventListener listener = new XinEventListener(event);
                    listener.addListener();
                    xinEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            xinEventListeners.forEach(XinEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Xin event listener
     */
    private class XinEventListener {

        /**
         * Event handler
         */
        private final XinEventHandler eventHandler;

        /**
         * Create the Xin event listener
         *
        ...
        /**
         * Add the Xin listener for this event
         */
        ...
        /**
         * Remove the Xin listener for this event
         */
        ...
        /**
         * Return the hash code for this Xin event listener
         *
         * @return Hash code
         */
        @Override
        ...
        /**
         * Check if two Xin events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof XinEventListener) &&
                    eventHandler.equals(((XinEventListener) obj).eventHandler));
        }

        /**
         * Xin listener event handler
         */
        private abstract class XinEventHandler {
             ...
             * @param eventRegistration Event registration
             */
            public XinEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof XinEventHandler) &&
                        owner == ((XinEventHandler) obj).owner &&
                        accountId == ((XinEventHandler) obj).accountId &&
                        event == ((XinEventHandler) obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends XinEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends XinEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends XinEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends XinEventHandler implements Listener<LedgerEntry> {
