...
public class DesktopSystemTray {
    ...
    void createAndShowGUI() {
        ...
        imageIcon = new ImageIcon("html/www/wallet/assets/images/favicon.png", "tray icon");
        ...
        if (!Xin.isDesktopApplicationEnabled()) {
        ...
        showDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("xindesktop.DesktopApplication").getMethod("launch").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("xindesktop.DesktopApplication failed to launch", exception);
            }
        });

        refreshDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("xindesktop.DesktopApplication").getMethod("refresh").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("xindesktop.DesktopApplication failed to refresh", exception);
            }
        });
    ...
    private void displayStatus() {                System.out.println("------ 444");
        ...
        addDataRow(statusPanel, "Application", Xin.APPLICATION);
        addDataRow(statusPanel, "Version", Xin.VERSION);
        //addDataRow(statusPanel, "Network", (Constants.isTestnet) ? "TestNet" : "MainNet");
        addDataRow(statusPanel, "Network", "MainNet");
        ...
        addDataRow(statusPanel, "User folder", String.valueOf(Paths.get(Xin.getUserHomeDir()).toAbsolutePath()));
        ...
        if (lastBlock != null) {
            ...
            addDataRow(statusPanel, "Seconds passed", String.valueOf(Xin.getEpochTime() - lastBlock.getTimestamp()));
        ...
        addDataRow(statusPanel, "Process id", Xin.getProcessId());
        addEmptyRow(statusPanel);
        ...
        if (statusDialog == null || !statusDialog.isVisible()) {
            ...
            statusDialog = pane.createDialog(wrapper, "IEP Server Status");
