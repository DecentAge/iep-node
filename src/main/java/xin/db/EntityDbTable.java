...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        ...
        //https://bitbucket.org/JeanLucPicard/nxt/commits/91074952dc702e482287df212df865cce58cafc6
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Xin.getBlockchainProcessor().isScanning() ?
                    Math.max(Xin.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
    ...
    private boolean doesNotExceed(int height) {
        return Xin.getBlockchain().getHeight() <= height && !(isPersistent() && Xin.getBlockchainProcessor().isScanning());
    }
}
