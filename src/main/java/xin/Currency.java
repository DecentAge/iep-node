...
public final class Currency {
    ...
    private static final class CurrencySupply {
        ...
        private long currentReservePerUnitTQT;
        ...
        private CurrencySupply(ResultSet rs, DbKey dbKey) throws SQLException {
            ...
            this.currentReservePerUnitTQT = rs.getLong("current_reserve_per_unit_tqt");
        }

        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO currency_supply (id, current_supply, "
                    + "current_reserve_per_unit_tqt, height, latest) "
                    + "KEY (id, height) VALUES (?, ?, ?, ?, TRUE)")) {
                int i = 0;
                pstmt.setLong(++i, this.currencyId);
                pstmt.setLong(++i, this.currentSupply);
                pstmt.setLong(++i, this.currentReservePerUnitTQT);
    ...
    public static DbIterator<Currency> getAllCurrencies(int from, int to, SortClause sortClause) {
        return currencyTable.getAll(from, to, sortClause.getClause());
    }

    public static DbIterator<Currency> getAllCrowdFundingCurrencies(Long accountId, int from, int to, SortClause sortClause) {
        DbClause issuanceClause = new DbClause.LongClause("issuance_height", DbClause.Op.GT, 0);
        DbClause finalClause = issuanceClause;
        if (accountId != null) {
            finalClause = finalClause.and(new DbClause.LongClause("account_id", accountId));
        }
        return currencyTable.getManyBy(finalClause, from, to,sortClause.getClause());
    ...
    static {
        Xin.getBlockchainProcessor().addListener(new CrowdFundingListener(), BlockchainProcessor.Event.AFTER_BLOCK_APPLY);
    }
    ...
    private Currency(Transaction transaction, Attachment.MonetarySystemCurrencyIssuance attachment) {
        ...
        this.minReservePerUnitTQT = attachment.getMinReservePerUnitTQT();
    ...
    private Currency(ResultSet rs, DbKey dbKey) throws SQLException {
        ...
        this.minReservePerUnitTQT = rs.getLong("min_reserve_per_unit_tqt");
    ...
    private void save(Connection con) throws SQLException {
        try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO currency (id, account_id, name, code, "
                + "description, type, initial_supply, reserve_supply, max_supply, creation_height, issuance_height, min_reserve_per_unit_tqt, "
            ...
            pstmt.setLong(++i, this.minReservePerUnitTQT);
    ...
    public long getCurrentReservePerUnitTQT() {
        if (!is(CurrencyType.RESERVABLE) || getSupplyData() == null) {
            return 0;
        }
        return currencySupply.currentReservePerUnitTQT;
    }
    ...
    static void increaseReserve(LedgerEvent event, long eventId, Account account, long currencyId, long amountPerUnitTQT) {
        Currency currency = Currency.getCurrency(currencyId);
        account.addToBalanceTQT(event, eventId, -Math.multiplyExact(currency.getReserveSupply(), amountPerUnitTQT));
        CurrencySupply currencySupply = currency.getSupplyData();
        currencySupply.currentReservePerUnitTQT += amountPerUnitTQT;
        currencySupplyTable.insert(currencySupply);
        CurrencyFounder.addOrUpdateFounder(currencyId, account.getId(), amountPerUnitTQT);
    }
    ...
    static void claimReserve(LedgerEvent event, long eventId, Account account, long currencyId, long units) {
        ...
        account.addToBalanceAndUnconfirmedBalanceTQT(event, eventId,
                Math.multiplyExact(units, currency.getCurrentReservePerUnitTQT()));
    }
                ...
                try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currencyId, 0, Integer.MAX_VALUE)) {
                    for (CurrencyFounder founder : founders) {
                        Account.getAccount(founder.getAccountId())
                                .addToBalanceAndUnconfirmedBalanceTQT(event, eventId, Math.multiplyExact(reserveSupply,
                                        founder.getAmountPerUnitTQT()));
                    }
                }
            }
    ...
    private static final class CrowdFundingListener implements Listener<Block> {
        ...
        public void notify(Block block) {
            ...
            try (DbIterator<Currency> issuedCurrencies = currencyTable.getManyBy(new DbClause.IntClause("issuance_height", block.getHeight()), 0, -1)) {
                for (Currency currency : issuedCurrencies) {
                    if (currency.getCurrentReservePerUnitTQT() < currency.getMinReservePerUnitTQT()) {
        ...
        private void undoCrowdFunding(Currency currency) {
            try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
                for (CurrencyFounder founder : founders) {
                    Account.getAccount(founder.getAccountId())
                            .addToBalanceAndUnconfirmedBalanceTQT(LedgerEvent.CURRENCY_UNDO_CROWDFUNDING, currency.getId(),
        ...
        private void distributeCurrency(Currency currency) {
            ...
            if (!currency.is(CurrencyType.CLAIMABLE)) {
                issuerAccount.addToBalanceAndUnconfirmedBalanceTQT(LedgerEvent.CURRENCY_DISTRIBUTION, currency.getId(),
