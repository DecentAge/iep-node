...
import xin.crypto.AnonymouslyEncryptedData;
...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Xin.getBooleanProperty("xin.deleteFinishedShufflings");
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (holdingType != HoldingType.XIN) {
                    participantAccount.addToBalanceTQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_TQT);
                }
            }
        }
        for (byte[] recipientPublicKey : recipientPublicKeys) {
            ...
            if (holdingType != HoldingType.XIN) {
                recipientAccount.addToBalanceAndUnconfirmedBalanceTQT(event, this.id, Constants.SHUFFLING_DEPOSIT_TQT);
            }
        }
    ...
    private void cancel(Block block) {
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (participantAccount.getId() != blamedAccountId) {
                    if (holdingType != HoldingType.XIN) {
                        participantAccount.addToUnconfirmedBalanceTQT(event, this.id, Constants.SHUFFLING_DEPOSIT_TQT);
                    }
                } else {
                    if (holdingType == HoldingType.XIN) {
                        participantAccount.addToUnconfirmedBalanceTQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_TQT);
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                previousGeneratorAccount.addToBalanceAndUnconfirmedBalanceTQT(AccountLedger.LedgerEvent.BLOCK_GENERATED, block.getId(), fee);
                previousGeneratorAccount.addToForgedBalanceTQT(fee);
                Logger.logDebugMessage("Shuffling penalty %f token awarded to forger at height %d", ((double) fee) / Constants.ONE_XIN, block.getHeight() - i - 1);
            }
            fee = Constants.SHUFFLING_DEPOSIT_TQT - 3 * fee;
            Account blockGeneratorAccount = Account.getAccount(block.getGeneratorId());
            blockGeneratorAccount.addToBalanceAndUnconfirmedBalanceTQT(AccountLedger.LedgerEvent.BLOCK_GENERATED, block.getId(), fee);
            blockGeneratorAccount.addToForgedBalanceTQT(fee);
            Logger.logDebugMessage("Shuffling penalty %f token awarded to forger at height %d", ((double) fee) / Constants.ONE_XIN, block.getHeight());
        }
