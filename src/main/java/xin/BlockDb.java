...
final class BlockDb {
    ...
    static final Blockchain blockchain = Xin.getBlockchain();
    ...
    static BlockImpl loadBlock(Connection con, ResultSet rs, boolean loadTransactions) {
        try {
            ...
            long totalAmountTQT = rs.getLong("total_amount");
            long totalFeeTQT = rs.getLong("total_fee");
            ...
            return new BlockImpl(version, timestamp, previousBlockId, totalAmountTQT, totalFeeTQT, payloadLength, payloadHash,
                    generatorId, generationSignature, blockSignature, previousBlockHash,
                    cumulativeDifficulty, baseTarget, nextBlockId, height, id, loadTransactions ? TransactionDb
                    .findBlockTransactions(con, id) : null,blockATs);
        } catch (SQLException e) {
            throw new RuntimeException(e.toString(), e);
        }
    }
    ...
    static void saveBlock(Connection con, BlockImpl block) {
        try {
            try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO block (id, version, timestamp, previous_block_id, "
                    ...
                    + "base_target, height, generation_signature, block_signature, payload_hash, generator_id, ats) "
                    + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)")) {
                int i = 0;
                ...
                pstmt.setLong(++i, block.getTotalAmountTQT());
                pstmt.setLong(++i, block.getTotalFeeTQT());
