...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Xin.APPLICATION.equals(application)) {
            String[] versions;
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Xin.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceTQT();
            hallmarkBalanceHeight = Xin.getBlockchain().getHeight();
        }
        return (int) (adjustedWeight * (hallmarkBalance / Constants.ONE_XIN) / Constants.MAX_BALANCE_XIN);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof XinException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
        ...
        if (!isBlacklisted()) {
            if (cause instanceof IOException || cause instanceof ParseException || cause instanceof IllegalArgumentException) {
                ...
                // Unexpected cause (e.g. BufferOverflowException): log at WARN with the
                // throwable so the stack trace reaches the console (the log4j2 console
                // appender is pinned at INFO, so logDebugMessage stays hidden).
    ...
    public void blacklist(String cause) {
        blacklistingTime = Xin.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/api"));
            //
            ...
            if (useWebSocket) {
                ...
                if (maxResponseSize > 0) {
                    ...
                    if (wsResponse.length() > maxResponseSize)
                        throw new XinException.XinIOException("Maximum size exceeded: " + wsResponse.length());
                ...
                //
                // URL url = new URL("http://" + host + ":" + getPort() + "/api");
        ...
        } catch (XinException.XinIOException e) {
            blacklist(e);
            if (connection != null) {
                connection.disconnect();
            }
    ...
    void connect() {
        lastConnectAttempt = Xin.getEpochTime();
        try {
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_XIN * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
